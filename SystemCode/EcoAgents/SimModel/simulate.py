...
def modify_profiles(city):
    with open('data/profiles.json', 'r') as f:
        profiles = json.load(f)
    
    profiles['City'][0] = city
...
def gpt_actions(env, obs, dialog_queue, dialog4ref_queue, gpt_path, gpt_error, total_cost):
    ...
    for idx in range(env.num_agents):
        ...
        education = this_agent.endogenous['education']
        ...
        problem_prompt = f'''
                    You're {name}, a {age}-year-old individual living in {city}, with {education} education. As with all Americans, a portion of your monthly income is taxed by the federal government. This taxation system is tiered, income is taxed cumulatively within defined brackets, combined with a redistributive policy: after collection, the government evenly redistributes the tax revenue back to all citizens, irrespective of their earnings.
        ...
        obs_prompt = f'''
                        ...
                        Warning: Only output a JSON, and any other output is not allowed.
                    '''
    ...
    def extract_dict_from_text(text):
        try:
            cleaned_text = re.sub(r'^```json\s*', '', text)  # Remove ```json and any leading whitespace
            cleaned_text = re.sub(r'\s*```$', '', cleaned_text)  # Remove trailing ```
            return json.loads(cleaned_text)
        except json.JSONDecodeError:
            print(f"Error decoding JSON: {cleaned_text}")
            return {}
    ...
    for idx in range(env.num_agents):
        content = results[idx]
        try:
            extracted_actions = list(extract_dict_from_text(content).values())
    ...
    for idx, agent_dialog in enumerate(dialog_queue):
        ...
        file_path = f'{gpt_path}/{agent_name}'
...
# python simulate.py --policy_model gpt --num_agents 100 --episode_length 240 --city "New York City" --year 2018
# python pkl2csv.py --policy_model gpt --num_agents 100 --episode_length 240 --city "New York City" --year 2018
def main(policy_model='gpt', num_agents=100, episode_length=240, dialog_len=3, beta=0.1, gamma=0.1, h=1, max_price_inflation=0.1, max_wage_inflation=0.05, city=None, year=2018):
    if city is None:
        city = "New York City"
    else:
        city = str(city)  # Convert to string in case it's not already
    modify_profiles(city)
    env_config['n_agents'] = num_agents
    env_config['episode_length'] = episode_length
    env_config['year'] = year
    if policy_model == 'gpt':
        total_cost = 0
