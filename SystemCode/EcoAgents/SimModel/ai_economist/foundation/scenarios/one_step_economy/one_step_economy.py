...
from pro_based_choose import main as choose_offer
...
class OneStepEconomy(BaseEnvironment):
    ...
    def set_offer(self):
        for agent in self.world.agents:
            if agent.state["endogenous"]['job'] == 'Unemployment':
                previous_job = agent.state["endogenous"]['previous_job']
                offer = choose_offer(previous_job, year=self.year)
                agent.state["endogenous"]['offer'] = offer
    ...
    def reset_agent_states(self):
        ...
        education_levels = profiles['Education']
        education_probabilities = [0.13, 0.2, 0.35, 0.2, 0.1, 0.02]
        agent_education = np.random.choice(education_levels, self.n_agents, p=education_probabilities)
        ...
        for idx, agent in enumerate(self.world.agents):
            ...
            agent.state["endogenous"]['education'] = agent_education[idx]
            agent.state["endogenous"]['job'] = 'Unemployment'
            agent.state["endogenous"]["previous_job"] = 'Unemployment'
    ...
    def scenario_metrics(self):
        ...
        for endogenous, quantities in agent_endogenous.items():
            if isinstance(quantities, list) and all(isinstance(item, (int, float)) for item in quantities):
                metrics["endogenous/avg_agent/{}".format(endogenous)] = np.mean(quantities)
            else:
                metrics["endogenous/avg_agent/{}".format(endogenous)] = quantities
